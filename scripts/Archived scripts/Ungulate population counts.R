...
#and moose (Alces alces) between 2001 and 2016. 
...
Pop3<-aggregate(Estimate~Year +Taxon.ID, data=Pop, FUN='mean')

plot2<-ggplot(data=Pop3, aes(x=Year, y=Estimate, group=Taxon.ID, colour=Taxon.ID))+
  geom_line()+
  geom_point()+
...
Pop1time<-ts(Pop1.1, start=2001, end=2016, frequency=1)###start is when the time series starts
