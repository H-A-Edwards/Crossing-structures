...
daynight.human = (
  Humanuse %>%
    group_by(Year, daynight, Location2, Underpass.type) %>%
    summarise(daynight.human = sum(Total))
)

daynight.human2 = (
  daynight.human %>%
    group_by(daynight, Location2, Underpass.type) %>%
    summarise(daynight.human = mean(daynight.human))
)

newdata = left_join(newdata, daynight.human2, by = c("daynight", "Location2", "Underpass.type"))

#-------------------------Average number of seasonal humans----------------------
seasonal.human = (
  Humanuse %>%
    group_by(Year, Season, Location2, Underpass.type) %>%
    summarise(seasonal.human = sum(Total))
)

seasonal.human2 = (
  seasonal.human %>%
    group_by(Season, Location2, Underpass.type) %>%
    summarise(seasonal.human = mean(seasonal.human))
)

newdata = left_join(newdata, seasonal.human2, by = c("Season","Location2", "Underpass.type"))
...
annual.human = (
  Humanuse %>%
    ...
    summarise(annual.human = sum(Total))
)

annual.human2 = (
  annual.human %>%
    group_by(Location2, Underpass.type) %>%
    summarise(annual.human = mean(annual.human))
)

newdata = left_join(newdata, annual.human2, by = c("Location2", "Underpass.type"))
